...
memory that shouldn't land in git. ``tara init`` creates a dedicated folder
...
by setting ``gitignore = false`` under ``[handover]`` in ``.tara/config.toml``.
...
# General per-repo tara settings (introduced for the handover opt-out).
...
_MARKER = "# Tara: agent memory / handover"


def load_config() -> dict:
    """Parse ``.tara/config.toml`` (empty dict if absent)."""
...
def _readme(folder: Path, gitignore: bool) -> str:
    ...
    status = (
        ...
        "`[handover] gitignore = false` in `.tara/config.toml`), so docs here "
    ...
    return (
        ...
        f"`gitignore = false` under `[handover]` in `.tara/config.toml` and "
