...
The freeform ``.agent/memory/`` folder (see :mod:`tara.handover`) holds prose
...
Tara only scaffolds the database (folder, empty schema, git-ignoring) and
...
by setting ``gitignore = false`` under ``[tracking]`` in ``.tara/config.toml``.
"""
...
from tara.handover import ensure_gitignored, load_config
...
_MARKER = "# Tara: agent hand-off log"
...
# Idempotent DDL tara applies when scaffolding; the runtime only INSERTs.
_SCHEMA_SQL = """\
...
def append_command(db: Path) -> str:
    ...
    Uses only the standard-library ``sqlite3`` module — no tara dependency —
...
def _readme(folder: Path, gitignore: bool) -> str:
    ...
    status = (
        ...
        "gitignore = false` in `.tara/config.toml` and removing the matching "
    ...
    return (
        ...
        f"flow's phase hand-offs: one SQLite table, one row per hand-off. Tara "
        ...
        f"Nothing here is a tara dependency: `sqlite3` is in the Python standard "
...
def _gitignore_block(folder: Path) -> str:
    ...
    return (
        ...
        f"# .tara/config.toml, then remove this block.\n"
        f"{rel}/{HANDOFFS_DB}\n"
